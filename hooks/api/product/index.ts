...
export const getProductByBrandIdApi = toQueryFetcher<
  TGetProductByBrandIdRequestParams,
  TServerResponse<TProduct[]>
>("getProductByBrandIdApi", async (params) => {
  return privateRequest(`/products/get-by-brand/${params?.brandId}`);
});

export const getAllProductApi = toQueryFetcher<
  void,
  TServerResponse<IResponseProduct[]>
>("getAllProductApi", async () => {
  return publicRequest("/products");
});
export const getProductApi = toQueryFetcher<string, TServerResponse<IProduct>>(
  "getProductApi",
  async (productId) => {
...
export const createProductApi = toMutationFetcher<
  IServerCreateProduct,
...
export const updateProductApi = toMutationFetcher<
  UpdateProductParams,
  TServerResponse<IServerCreateProduct>
>("updateProductApi", async ({ productId, data }: UpdateProductParams) => {
...
export const getProductFilterApi = toQueryFetcher<
  FilterParamProducts,
  TServerResponse<
    { total: string; limit: string; page: string },
    IResponseProduct[]
  >
