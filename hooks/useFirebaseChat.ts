 ...
 */
export const useFirebaseChat = (livestreamId: string, messageLimit = DEFAULT_MESSAGE_LIMIT) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const chatRef = firestore().collection('livestreams').doc(livestreamId).collection('messages')
          ...
          (snapshot) => {
            ...
            snapshot.docs.reverse().forEach((doc) => {
              ...
              newMessages.push({
                ...
                avatar: data.avatar || data.user?.substring(0, 2)?.toUpperCase() || 'AN',
                timestamp
              })
            })
  ...
  const sendMessage = useCallback(
    async (message: string) => {
      ...
      try {
        ...
        await firestore().collection('livestreams').doc(livestreamId).collection('messages').add({
          user: userName,
          userId,
