...
import {
  axiosBaseOptions,
  axiosProvincesOptions,
} from "@/network/axios/axios-setup";
...
class MyAxios {
  ...
  urlDownload(params: UrlDownload) {
    const {
      fileName,
      serveBaseUrl = process.env.EXPO_PUBLIC_API_URL,
      fileUrl,
    } = params;
    const a = document.createElement("a");
    a.style.display = "none";
...
export const axiosProvincesRequest =
  provincesRequest.axiosRequest.bind(provincesRequest);
