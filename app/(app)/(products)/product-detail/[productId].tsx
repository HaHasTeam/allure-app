...
import { filterFeedbackApi } from "@/hooks/api/feedback/index";
...
const ProductDetailScreen = ({
  ...
  const defaultClassification = useMemo(() => {
    ...
    return (
      product.productClassifications.find((c) => c.type === "DEFAULT") || null
    );
  }, [product?.productClassifications]);

  const cheapestClassification = useMemo(() => {
    if (!product?.productClassifications?.length) return null;
    return [...product.productClassifications].sort(
      (a, b) => a.price - b.price
    )[0];
  }, [product?.productClassifications]);
  ...
  const shouldShowSizeSelection = useMemo(() => {
    return (
      (product?.productClassifications?.length ?? 0) > 0 &&
  ...
  const deal =
    event === OrderEnum.FLASH_SALE
      ? product?.productDiscounts?.[0]?.discount || 0
      : 0;
  const originalPrice =
    selectedSize?.price || cheapestClassification?.price || product?.price || 0;
  const currentPrice =
    deal > 0
      ? calculateDiscountPrice(originalPrice, deal, DiscountTypeEnum.PERCENTAGE)
      : originalPrice;
  ...
  const getLabel = (key: string, value: string) => {
    ...
    if (
      !category ||
      (category.type !== "singleChoice" && category.type !== "multipleChoice")
    ) {
      return value;
    }

    const matchingOption = category?.options?.find(
      (opt) => opt.value === value
    );
  ...
  const renderReviewItem = (review: IResponseFilterFeedback) => {
    ...
    const username =
      review.orderDetail.order.account.username || "Anonymous User";
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerActions}>
          {/* Removed favorite and share buttons */}
        </View>
      </View>
          ...
          <Animated.FlatList
            ...
            renderItem={({ item, index }) => (
              <Pressable
                ...
                onPress={toggleFullscreenGallery}
              >
                <Image
                  source={{
                    uri:
        ...
        <View style={styles.detailsContainer}>
              ...
              <View style={styles.specialEventInfo}>
                ...
                <Text style={styles.specialEventTime}>
                  {event === OrderEnum.FLASH_SALE
                    ? t("flashSale.endsIn", {
                        val: new Date(
                          product?.productDiscounts?.[0]?.endTime || Date.now()
                        ),
                      })
                    : t("preOrder.endsIn", {
                        val: new Date(
                          product?.preOrderProducts?.[0]?.endTime || Date.now()
                        ),
                      })}
                </Text>
              </View>
          ...
          {product?.productDiscounts?.[0]?.status ===
            ...
            <View style={styles.waitingContainer}>
              <Text style={styles.waitingText}>
                {t("flashSale.waiting", {
                  val: new Date(
                    product?.productDiscounts?.[0]?.startTime || Date.now()
                  ),
                })}
              </Text>
            </View>
            ...
            {showPriceDetails && (
              <View style={styles.priceDetailsPopup}>
                ...
                <View style={styles.priceDetailsContent}>
                  ...
                  <View
                    style={[styles.priceDetailRow, styles.priceDetailTotal]}
                  >
                    <Text style={styles.priceDetailTotalLabel}>
                      {t("cart.estimateProductPrice")}
                    </Text>
                    <Text style={styles.priceDetailTotalValue}>
                      {t("productCard.price", { price: currentPrice })}
                    </Text>
                  </View>
                </View>
              </View>
          ...
          <View style={styles.qualityServiceContainer}>
            <Text style={styles.sectionTitle}>
              {t("productDetail.qualityService")}
            </Text>
              ...
              {product.certificates && product.certificates.length > 0 && (
                <View style={styles.qualityServiceItem}>
                  ...
                  <View style={styles.qualityServiceContent}>
                    <Text style={styles.qualityServiceText}>
                      {t("productDetail.safeCertificate")}
                    </Text>
                    <View style={styles.certificateActions}>
                      {product.certificates.map((cert, index) => (
                        <View key={index} style={styles.certificateAction}>
                          {index > 0 && (
                            <Text style={styles.certificateSeparator}>|</Text>
                          )}
