...
import { Alert, FlatList, StyleSheet, TouchableOpacity, ScrollView } from 'react-native'
...
const ProfileScreen = () => {
  ...
  const settingMenu = [
    ...
    {
      icon: <MaterialCommunityIcons style={style.icon} name='logout' size={24} color={myTheme.red[500]} />,
        ...
        <Card style={style.profileCard}>
          {loading ? (
            <View style={style.loadingContainer}>
              <MyText text='Đang tải thông tin...' styleProps={{ fontSize: 16, color: myTheme.grey }} />
            </View>
          ) : (
            <>
              <View style={style.profileHeader}>
                ...
                <View style={style.nameContainer}>
                  ...
                  <View style={style.usernameRow}>
                    ...
                    />
                    {user?.gender && <View style={{ marginLeft: 8 }}>{getGenderIcon(user.gender)}</View>}
                  </View>
                  <View style={style.statusBadge}>
                    <View style={[style.statusDot, { backgroundColor: getStatusColor(user?.status || '') }]} />
                    <MyText
                      text={user?.status || ''}
                      styleProps={{
                        fontSize: 12,
              ...
              <View style={style.detailsContainer}>
                <View style={style.detailRow}>
                  <MaterialIcons name='email' size={20} color={myTheme.primary} />
                  <MyText text={user?.email || 'N/A'} styleProps={{ fontSize: 14, marginLeft: 10 }} />
                  {user?.isEmailVerify && (
                    <View style={style.verifiedBadge}>
                      <MaterialIcons name='verified' size={16} color={myTheme.green[500]} />
                    </View>
                  )}
                </View>
                ...
                <View style={style.detailRow}>
                  ...
                  <MyText text={user?.phone || 'Chưa cập nhật'} styleProps={{ fontSize: 14, marginLeft: 10 }} />
                </View>
                ...
                <View style={style.detailRow}>
                  <MaterialIcons name='badge' size={20} color={myTheme.primary} />
                  <MyText text={user?.role || 'N/A'} styleProps={{ fontSize: 14, marginLeft: 10 }} />
                </View>
              </View>
            </>
          )}
