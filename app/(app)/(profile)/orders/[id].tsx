...
const OrderDetail = () => {
  ...
  const { data: useStatusTrackingData, isFetching: isFetchingStatusTracking } =
    useQuery({
      queryKey: [
  ...
  const pendingRequestCancelTime = useMemo(() => {
    return millisecondsToRoundedDays(
      parseInt(masterConfig?.data[0].autoApprovedRequestCancelTime ?? "")
    );
  }, [masterConfig?.data]);
  ...
  const pendingRequestReturnTime = useMemo(() => {
    return (
      ...
      millisecondsToRoundedDays(
        parseInt(masterConfig?.data[0].autoApproveRefundRequestTime ?? "") +
          parseInt(
            masterConfig?.data[0].pendingAdminCheckRejectRefundRequestTime ?? ""
          )
      )
    );
  }, [masterConfig?.data]);
  const pendingCustomerShippingReturnTime = useMemo(() => {
    return millisecondsToRoundedDays(
      parseInt(masterConfig?.data[0].pendingCustomerShippingReturnTime ?? "")
    );
  }, [masterConfig?.data]);
  return (
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.headerContainer}>
          ...
          {!isFetching && useOrderData?.data && (
            <View style={styles.statusContainer}>
              <MyText
                text={`${t("orderDetail.status")}: `}
                styleProps={styles.statusLabelText}
              />
              <OrderStatus
                tag={useOrderData?.data?.status ?? ""}
                size="small"
              />
            </View>
          )}
        ...
        {!isFetching && useOrderData && useOrderData?.data && (
          <View style={styles.contentContainer}>
            ...
            {!isFetchingStatusTracking &&
              useStatusTrackingData &&
              useStatusTrackingData?.data && (
                <OrderStatusTracking
            ...
            {!isLoading &&
              cancelAndReturnRequestData?.data?.complaintRequest?.status ===
            ...
            <View style={styles.infoSection}>
              <View style={styles.timelineSection}>
                <OrderGeneral
                  ...
                  icon={
                    <Octicons name="history" size={24} color={myTheme.muted} />
                  }
                  content={
                    !isFetchingStatusTracking &&
                    useStatusTrackingData &&
              ...
              <View style={styles.addressSection}>
                <OrderGeneral
                  ...
                  icon={
                    <Feather name="truck" size={24} color={myTheme.muted} />
                  }
                  content={
                    <View style={styles.addressContent}>
                      <View style={styles.addressRow}>
                        <MyText
                          text={`${t("orderDetail.recipientName")}: `}
                          styleProps={styles.addressLabel}
                        />
                        <MyText
                          text={useOrderData?.data?.recipientName}
                          styleProps={styles.addressText}
                        />
                      </View>
                      <View style={styles.addressRow}>
                        <MyText
                          text={`${t("orderDetail.address")}: `}
                          styleProps={styles.addressLabel}
                        />
                        <MyText
                          text={useOrderData?.data?.shippingAddress}
                          styleProps={styles.addressText}
                        />
                      </View>
                      <View style={styles.addressRow}>
                        <MyText
                          text={`${t("orderDetail.phone")}: `}
                          styleProps={styles.addressLabel}
                        />
                        <MyText
                          text={useOrderData?.data?.phone}
                          styleProps={styles.addressText}
                        />
                      </View>
                      <View style={styles.addressRow}>
                        <MyText
                          text={`${t("orderDetail.notes")}: `}
                          styleProps={styles.addressLabel}
                        />
                        <MyText
                          text={
            ...
            <View style={styles.orderDetails}>
              ...
              <OrderSummary
                ...
                totalPlatformDiscount={
                  useOrderData?.data?.platformVoucherDiscount
                }
      ...
      {!isFetching &&
        ...
        cancelAndReturnRequestData?.data?.refundRequest && (
          <ConfirmDecisionDialog
            ...
            mediaFiles={
              cancelAndReturnRequestData?.data?.refundRequest.mediaFiles
            }
            isRejectRequest={
              cancelAndReturnRequestData?.data?.refundRequest
                ?.rejectedRefundRequest !== null
            }
            ...
            reasonRejected={
              cancelAndReturnRequestData?.data?.refundRequest
                ?.rejectedRefundRequest?.reasonRejected
            }
            rejectTime={
              cancelAndReturnRequestData?.data?.refundRequest
                ?.rejectedRefundRequest?.createdAt
            }
            reviewTime={
              cancelAndReturnRequestData?.data?.refundRequest
            ...
            returnTime={
              cancelAndReturnRequestData?.data?.refundRequest?.createdAt
            }
          />
        )}
      {!isFetching &&
        ...
        cancelAndReturnRequestData?.data?.complaintRequest && (
          <ConfirmDecisionDialog
            ...
            mediaFiles={
              cancelAndReturnRequestData?.data?.complaintRequest?.mediaFiles
            }
            ...
            reasonRejected={
              cancelAndReturnRequestData?.data?.complaintRequest?.reasonRejected
            }
            reviewTime={
              cancelAndReturnRequestData?.data?.complaintRequest?.updatedAt
            }
            returnTime={
              cancelAndReturnRequestData?.data?.complaintRequest.createdAt
            }
          />
        )}
    </View>
