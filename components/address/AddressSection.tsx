...
export default function AddressSection({ addresses, setValue }: AddressSectionProps) {
      ...
      <View style={{ width: '100%', marginBottom: 8 }}>
        ...
        >
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            <MaterialCommunityIcons name='map-marker-check' size={20} color={myTheme.mutedForeground} />
            <Text style={{ fontSize: 18, color: myTheme.mutedForeground }}>{t('address.deliveryTo')}</Text>
          </View>
          <TouchableOpacity onPress={() => toggleModalVisibility()}>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <MaterialCommunityIcons name='pencil' size={16} color={myTheme.blue[500]} />
              <Text style={{ color: myTheme.blue[500] }}>{t('address.edit')}</Text>
            </View>
          </TouchableOpacity>
        </View>
        {chosenAddress ? (
          <View style={{ marginTop: 8 }}>
            ...
            <View style={{ flexDirection: 'row', gap: 6 }}>
              ...
              <Text style={{ color: myTheme.mutedForeground, width: '80%' }}>{chosenAddress?.fullAddress}</Text>
            </View>
          </View>
        ) : (
