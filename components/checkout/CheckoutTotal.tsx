    ...
    <View style={styles.container}>
      <View style={styles.detailsContainer}>
        ...
        <View style={styles.rowBetween}>
          ...
          <Text style={styles.mediumText}>{t('productCard.price', { price: totalProductCost })}</Text>
        </View>
        ...
        {totalProductDiscount && totalProductDiscount > 0 ? (
          <View style={styles.rowBetween}>
            ...
            <Text style={styles.discountText}>-{t('productCard.price', { price: totalProductDiscount })}</Text>
          </View>
        ) : null}
        {totalBrandDiscount && totalBrandDiscount > 0 ? (
          <View style={styles.rowBetween}>
            <Text style={styles.mutedText}>{t('cart.discountBrand')}</Text>
            <Text style={styles.discountText}>-{t('productCard.price', { price: totalBrandDiscount })}</Text>
          </View>
        ) : null}
        ...
        <View style={styles.totalPaymentContainer}>
          <Text style={styles.totalPaymentLabel}>{groupBuying ? t('cart.maxPrice') : t('cart.totalPayment')}</Text>
        ...
        {totalSavings && totalSavings > 0 ? (
          <View style={styles.savingsContainer}>
            ...
            <Text style={styles.savingsText}>-{t('productCard.price', { price: totalSavings })}</Text>
          </View>
        ) : null}
      </View>
      <View style={styles.footerContainer}>
        <Text style={styles.descriptionText}>{t('cart.checkoutDescription')}</Text>
        ...
        <TouchableOpacity
          style={[
            styles.submitButton,
            {
              backgroundColor: isLoading ? hexToRgba(myTheme.destructive, 0.8) : myTheme.destructive
            }
          ]}
