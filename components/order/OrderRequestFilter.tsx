    ...
    return (
      ...
      >
        <View style={styles.badgeContent}>
          <MyText text={item?.label || ''} styleProps={{ fontSize: 12, color: textColor }} />
          <MyText text='×' styleProps={{ fontSize: 12, color: textColor, marginLeft: 4 }} />
        </View>
      </TouchableOpacity>
    )
  }
    ...
    <Picker.Item
      ...
      renderItem={(selected) => (
        <View style={styles.pickerItem}>
          {selected && <Feather name='check' size={16} style={styles.checkIcon} />}
      ...
      {(typeFilters.length > 0 || statusFilters.length > 0) && (
        <View style={styles.filtersContainer}>
          <FlatList
            ...
            renderItem={({ item }) =>
              item.type === 'type'
                ? renderBadge(item.value, requestTypes, toggleType, myTheme.purple[100], myTheme.purple[600])
                : renderBadge(item.value, requestStatuses, toggleStatus, myTheme.orange[100], myTheme.orange[600])
            }
            ListFooterComponent={() =>
              typeFilters.length > 0 || statusFilters.length > 0 ? (
                <TouchableOpacity style={styles.clearButton} onPress={clearFilters}>
                  <MyText text={t('filter.reset')} styleProps={{ color: myTheme.gray[500] }} />
                </TouchableOpacity>
              ) : null
            }
          />
