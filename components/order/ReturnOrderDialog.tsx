...
import { getOrderByIdApi, getStatusTrackingByIdApi, updateOrderStatusApi } from '@/hooks/api/order'
...
export const ReturnOrderDialog: React.FC<ReturnOrderDialogProps> = ({
  ...
  const { mutateAsync: updateOrderStatusFn } = useMutation({
    ...
    onSuccess: async () => {
      showToast(t('return.returnOrderEvidenceDialog.returnOrderShipmentSuccessMessage'), 'success', 4000)
      await Promise.all([
  ...
  return (
    ...
    >
      ...
      <BottomSheetView style={styles.contentContainer}>
        <Text style={styles.heading}>{t('return.returnOrderEvidenceDialog.title')}</Text>
        <Text style={styles.textJustify}>{t('return.returnOrderEvidenceDialog.description')}</Text>
        ...
        <ScrollView>
          ...
          <View style={styles.mediaContainer}>
            <Text style={[styles.label, { color: myTheme.primary }]}>{t('feedback.mediaFiles')} *</Text>
            <Text style={styles.formDescription}>{t('return.returnOrderEvidenceDialog.mediaFilesNotes')}</Text>
          ...
          <View style={styles.formFieldsContainer}>
            <Controller
              ...
              render={({ field }) => (
                <View style={styles.fieldContainer}>
                  <Text style={[styles.label, { color: myTheme.primary }]}>{t('feedback.uploadVideos')} *</Text>
                  <UploadMediaFiles
                    field={field}
                    vertical={false}
                    isAcceptImage={false}
                    ...
                    renderInputUI={(_isDragActive, files, maxFiles) => (
                      <View style={styles.uploadContainer}>
                        ...
                        <Text style={[styles.uploadText, { color: myTheme.mutedForeground }]}>
                          {files.length}/{maxFiles} {t('media.videosFile')}
                        </Text>
                      </View>
                    )}
                  />
                  {errors.videos && <Text style={styles.errorText}>{errors.videos.message}</Text>}
                </View>
              )}
            />
            ...
            <Controller
              ...
              render={({ field }) => (
                <View style={styles.fieldContainer}>
                  <Text style={[styles.label, { color: myTheme.primary }]}>{t('feedback.uploadImages')} *</Text>
                  <UploadMediaFiles
                    field={field}
                    vertical={false}
                    isAcceptImage
                    ...
                    renderInputUI={(_isDragActive, files, maxFiles) => (
                      <View style={styles.uploadContainer}>
                        <Feather name='image' size={32} color={myTheme.primary} />
                        <Text style={[styles.uploadText, { color: myTheme.mutedForeground }]}>
          ...
          <View style={styles.footer}>
            ...
            >
              <Text style={[styles.buttonText, { color: myTheme.primary }]}>{t('button.cancel')}</Text>
            </TouchableOpacity>
            <TouchableOpacity
