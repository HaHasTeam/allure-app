...
const UploadMediaFiles = <T extends FieldValues>({
  ...
  const checkFileLimits = (files: File[]) => {
    ...
    if (maxImages && images.length > maxImages) {
      handleServerError({
        error: new Error(
          t("validation.maxImagesExceeded", { count: maxImages })
        ),
      });
      return false;
    }
    ...
    if (maxVideos && videos.length > maxVideos) {
      handleServerError({
        error: new Error(
          t("validation.maxVideosExceeded", { count: maxVideos })
        ),
      });
      return false;
    }
  ...
  const onFileDrop = async (newFiles: File[] | null) => {
    ...
    try {
      ...
      if (fieldType === "string") {
        ...
        if (newFiles?.length) {
          field?.onChange?.(
            newFiles[0] as unknown as React.ChangeEvent<HTMLInputElement>
          );
        }
        return;
      }
      ...
      if (fieldType === "array" && field?.value) {
        ...
        if (newFiles.length > oldFiles.length) {
          ...
          field?.onChange?.(
            updatedFiles as unknown as React.ChangeEvent<HTMLInputElement>
          );
        }
        // Removing files
        else {
  ...
  const getAcceptedFormatsMessage = () => {
    ...
    })}. ${t("validation.fileFormat")} ${formats}. ${t(
      "validation.sizeFileValid",
    ...
    <View>
      ...
      <View
        style={[
          styles.container,
          isDragActive
            ? styles.activeDragContainer
            : styles.inactiveDragContainer,
        ]}
      >
        <FileUploader
          value={files}
        ...
        >
          <View style={styles.flexWrap}>
            <FileUploaderContent
              style={centerItem ? styles.centered : styles.start}
            >
                  ...
                  <FileInput>
                    {renderInputUI ? (
                      ...
                      <View style={styles.uploadBox}>
                        ...
                        {isDragActive ? (
                          <MyText
                            text={t("createProduct.dropFile")}
                            styleProps={styles.dropFileText}
                          />
                        ) : (
                          <View>
                            <MyText
                              text={t("createProduct.dragAndDrop")}
                              styleProps={styles.dragDropText}
                            />
                            <MyText
                              text={t("createProduct.selectFile")}
                              styleProps={styles.selectFileText}
                            />
                            {!fileStats.maxFilesReached ? (
                              <MyText
                                text={message}
                                styleProps={styles.messageText}
                              />
                            ) : (
                              <MyText
                                text={t("createProduct.reachMaxFiles")}
                                styleProps={styles.messageText}
                              />
                            )}
                            <MyText
                              text={`${t("validation.imageCount", {
                                defaultValue: "Images",
                              })}: ${fileStats.imageCount}/${
                                maxImages || "∞"
                              }, ${t("validation.videoCount", {
                                defaultValue: "Videos",
