...
const UploadFiles = ({
  ...
  const { mutateAsync: uploadFilesFn, isPending: isUploadingFiles } =
    useMutation({
  ...
  const { fieldType, fieldValue } = useMemo<TFieldFile>(() => {
    ...
    throw new Error(
      "Invalid field value. Must be either 'single' or 'multiple'"
    );
      ...
      try {
        if (fieldType === "single" && fieldValue) {
          if (
            ...
            (!!fieldValue.fileUrl && files.length === 1)
          ) {
            return;
          }
  ...
  const onFileDrop = async (newFiles: CustomFile[] | null) => {
    try {
      if (fieldType === "single") {
        ...
        if (field.onChange)
          field.onChange(
            fileItem as unknown as React.ChangeEvent<HTMLInputElement>
          );
      ...
      if (fieldType === "multiple") {
        ...
        if (newFiles && newFiles.length < files.length) {
          for (let i = 0; i < files.length; i++) {
            if (
              files[i].id &&
              !newFiles.find((file) => file.id === files[i].id)
            ) {
        ...
        const combineFiles = newFiles
          ? [...newFiles, ...deleteFiles]
          : [...deleteFiles];
        ...
        if (field.onChange)
          field.onChange(
            fileValues as unknown as React.ChangeEvent<HTMLInputElement>
          );
  ...
  const handleUploadFiles = useCallback(async () => {
    try {
      ...
      files.forEach((file) => {
        if (
          !!file.fileUrl &&
          file.fileUrl.includes("https://firebasestorage.googleapis.com/")
        )
          return;
      ...
      const constructedFiles: TFile[] = await uploadFilesFn(formData).then(
        (res) => {
      ...
      if (field.onChange) {
        if (fieldType === "single") {
          field.onChange(
            constructedFiles[0] as unknown as React.ChangeEvent<HTMLInputElement>
          );
        }
        if (fieldType === "multiple") {
          field.onChange(
            constructedFiles as unknown as React.ChangeEvent<HTMLInputElement>
          );
        }
      }
    } catch (error) {
