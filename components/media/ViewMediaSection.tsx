...
export default function ViewMediaSection({
  mediaFiles,
}: ViewMediaSectionProps) {
  const { t } = useTranslation();
  const [isVisible, setIsVisible] = useState(false);

  const videoFiles = mediaFiles.filter(
    (file: TServerFile) => file.fileUrl && isVideoFile(file.fileUrl)
  );
  const imageFiles = mediaFiles.filter(
    (file: TServerFile) => file.fileUrl && isImageFile(file.fileUrl)
  );
