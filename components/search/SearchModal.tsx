...
const SearchModal = ({
  ...
  const renderProductItem = ({ item }: { item: IResponseProduct }) => {
    ...
    const productClassification = getCheapestClassification(item.productClassifications ?? [])
    ...
    return (
      ...
      >
        <View style={styles.productImageContainer}>
          <Image source={{ uri: mockProduct.images?.[0]?.fileUrl }} style={styles.productImage} />
        </View>
        <View style={styles.productContent}>
          <Text style={styles.productName} numberOfLines={1}>
            {mockProduct.name}
          </Text>
          <Text style={styles.productPrice}>{t('productCard.price', { price: mockProduct?.price })}</Text>
        </View>
      </TouchableOpacity>
    )
  }
      ...
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.searchInputContainer}>
            <Feather name='search' size={18} color={myTheme.primary} style={styles.searchIcon} />
            <TextInput
              ref={inputRef}
