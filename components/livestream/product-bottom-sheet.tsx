...
import type { IResponseProduct, IProduct } from '@/types/product'
...
const ProductSelectionBottomSheet = ({
  ...
  const [classificationSheetVisible, setClassificationSheetVisible] = useState(false)
  const [currentAction, setCurrentAction] = useState<'cart' | 'buy'>('cart')
  const [selectedProduct, setSelectedProduct] = useState<any>(null)
  const [selectedLivestream, setSelectedLivestream] = useState<LiveSteamDetail | null>(null)
  ...
  const { mutateAsync: createCartItemFn, isPending: isAddingToCart } = useMutation({
  ...
  const handleAddToCart = async (product: any, livestreamDetail: LiveSteamDetail) => {
    // Check if product has classifications
    if (!product.classifications || product.classifications.length === 0) {
      showToast(t('cart.noClassificationError') || 'Product has no classification', 'error', 4000)
      return
    }

    // Check if there's a DEFAULT classification
    const defaultClassification = product.classifications.find((c: IClassification) => c.type === 'DEFAULT')
  ...
  const handleBuyNow = async (product: any, livestreamDetail: LiveSteamDetail) => {
    // Check if product has classifications
    if (!product.classifications || product.classifications.length === 0) {
      showToast(t('cart.noClassificationError') || 'Product has no classification', 'error', 4000)
      return
  ...
  const handleAddToCartWithClassification = async (classificationId: string, quantity: number, title?: string) => {
    try {
  ...
  const handleBuyNowWithClassification = async (classificationId: string, quantity: number, title?: string) => {
    try {
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptySubtext}>There are no products to display at this time</Text>
    </View>
  )
        ...
        <BottomSheetView style={[styles.contentContainer, { paddingBottom: insets.bottom }]}>
          ...
          {products.length === 0 ? (
            ...
            <FlatList
              ...
              renderItem={({ item }) => {
                const productClassifications = item?.product.productClassifications?.filter(
                  (classification) => classification.status === StatusEnum.ACTIVE
                )
                const productClassification = getCheapestClassification(item.product.productClassifications ?? [])
