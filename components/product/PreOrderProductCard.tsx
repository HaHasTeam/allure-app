...
function PreOrderProductCard({ preOrderProduct }: ProductCardProps) {
  ...
  const renderButton = () => {
    if (timeStatus === 'upcoming') {
      ...
      return (
        <TouchableOpacity style={[styles.button, { backgroundColor: myTheme.amber[500] }]}>
  ...
  return (
    <TouchableOpacity
      style={styles.card}
      onPress={() => router.push(`/(products)/product-detail/${preOrderProduct.product.id}`)}
    >
      ...
      <View style={styles.cardContent}>
        <ImageWithFallback
          source={{
            uri: preOrderProduct.productClassifications[0]?.images[0]?.fileUrl || '/placeholder.svg'
          }}
          style={styles.image}
          resizeMode='cover'
        />
