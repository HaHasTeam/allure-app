...
export function getOrderDetailDiscountPrice(orderDetail: {
  ...
  const { platformVoucherDiscount, shopVoucherDiscount, subTotal } =
    orderDetail;
...
export const calculatePaymentCountdown = (
  order: IOrder,
  masterConfigData: IMasterConfig[]
) => {
  const createdAt = new Date(order.createdAt).getTime();

  const autoCancelTime =
    parseInt(masterConfigData[0]?.autoCancelOrderTime ?? "") || 0;
