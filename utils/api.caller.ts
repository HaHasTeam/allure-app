...
import axios, { type AxiosError, type AxiosInstance, type AxiosResponse, type Method } from 'axios'
 ...
 */
export const showErrorAlert = (error: ApiError, retryAction?: () => void, goBackAction?: () => void) => {
  const buttons = []
  ...
  if (error.status === 401) {
    Alert.alert('Session Expired', 'Your session has expired. Please log in again to continue.', buttons)
  } else if (error.status === 403) {
    Alert.alert('Access Denied', error.message, buttons)
  } else if (error.status === 0) {
    Alert.alert('Connection Error', 'Please check your internet connection and try again.', buttons)
  } else {
